...
class FallbackLogger extends MarkerIgnoringBase {

    private static final String CONF_FILE = "fallback.properties";
    ...
    public static final String SYSTEM_PREFIX = "org.slf4j.fallback.";
    ...
    static void init() {
        ...
        if (DATE_TIME_FORMAT_STR != null) {
            try {
                ...
                Util.report("Bad date format in " + CONF_FILE + "; will output relative time", e);
            }
        }
    }
    ...
    private static void loadProperties() {
        try (InputStream in = FallbackLogger.class.getResourceAsStream(CONF_FILE)) {
            if (in == null)
                return;
    ...
    private void log(int level, String message, Throwable t) {
        ...
        switch (level) {
            case LOG_LEVEL_TRACE -> buf.append("TRACE");
            case LOG_LEVEL_DEBUG -> buf.append("DEBUG");
            case LOG_LEVEL_INFO -> buf.append("INFO");
            case LOG_LEVEL_WARN -> buf.append(WARN_LEVEL_STRING);
            case LOG_LEVEL_ERROR -> buf.append("ERROR");
        }
        if (LEVEL_IN_BRACKETS)
            buf.append(']');
        buf.append(' ');
